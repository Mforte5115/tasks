...
export function isValid(question: Question, answer: string): boolean {
    ...
    if (question.options.find((ans: string): boolean => ans === answer)) {
        return true;
    }
    return false;
}
...
export function toMarkdown(question: Question): string {
    const markDown: string = "# " + question.name + "\n" + question.body;
    if (question.type === "multiple_choice_question") {
        const markDownMult = markDown + "\n- " + question.options.join("\n- ");
        return markDownMult;
    }
    return markDown;
}
...
export function publishQuestion(question: Question): Question {
    const newQuest = { ...question, published: !question.published };
    return newQuest;
}
